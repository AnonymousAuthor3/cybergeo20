...
terms1 = read.csv("data/docprobas2.csv", sep=",", dec=".")
...
load("data/themesLDA.Rdata")
files$name = NULL
files$path = NULL

#'
#' @description keywords and themes from LDA
terms2 = read.csv("data/20themes20words.csv", sep=",", dec=".")

nameThemes = c(as.character(terms2$NAME), "Other")
...
pattern_list <- c("espace", "territoire", "environnement", "soci??t??", "r??seau", "interaction", "am??nagement", "urbanisme", "carte", "mod??le", "syst??me", "SIG", "fractale", "durabilit??", "repr??sentation", "migration", "quantitatif", "qualitatif", "post-moderne")
...
chronogram <- function(patterns) {
  ggplot(articles_matched(patterns), aes(date, articles)) +
    ...
    labs(title="Chronogramme des articles publi??s dans Cybergéo", x = "Année de publication", y = "Nombre d'articles publiés")
}
